...
function formObject(form){const out=Object.fromEntries(new FormData(form)); for(const el of form.querySelectorAll('[type=checkbox]')) out[el.name]=el.checked; out.offers=['basic','standard','premium'].map(code=>({code,name:code[0].toUpperCase()+code.slice(1),priceMinor:Math.round(Number(out[`offer_${code}_price`]||0)*100),description:out[`offer_${code}_description`]||'',deliveryDays:Number(out[`offer_${code}_delivery`]||out.deliveryDays||1),revisionsIncluded:Number(out[`offer_${code}_revisions`]??out.revisionsIncluded??0)})).filter(x=>x.priceMinor>0||x.description.trim()); return out;}
...
function serviceEditor(item={}){
  ...
  showDialog(`
    ...
    <form id="serviceForm" class="editor service-editor" novalidate>
      ...
      <fieldset><legend><span>4</span> Forfaits proposés</legend>
        <p class="muted-copy">Définissez jusqu’à trois niveaux. Le forfait Basic reprend le prix principal si vous laissez les autres vides.</p>
        <div class="offer-grid">${['basic','standard','premium'].map(code=>{const offer=(item.offers||[]).find(x=>x.code===code)||{};return `<label>${code[0].toUpperCase()+code.slice(1)} · Prix HTG<input name="offer_${code}_price" type="number" min="0" step="0.01" value="${offer.priceMinor?offer.priceMinor/100:(code==='basic'&&item.priceMinor?item.priceMinor/100:'')}"></label><label>Description<textarea name="offer_${code}_description" rows="2">${esc(offer.description||'')}</textarea></label><label>Délai (jours)<input name="offer_${code}_delivery" type="number" min="1" max="365" value="${offer.deliveryDays||item.deliveryDays||''}"></label><label>Révisions<input name="offer_${code}_revisions" type="number" min="0" max="50" value="${offer.revisionsIncluded??item.revisionsIncluded??0}"></label>`;}).join('')}</div>
      </fieldset>
      <fieldset><legend><span>5</span> Image de présentation</legend>
