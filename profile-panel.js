...
class ProfilePanel {
  ...
  renderWhatsAppView(colors, fonts) {
    ...
    const allDepartmentsSelected = this.whatsappDepartments.length > 0
      && this.whatsappDepartments.every((department) => selected.has(department.id));
    ...
    return `
      <section class="sc-wa-preferences" style="--sc-wa-title:${colors.text.title};--sc-wa-body:${colors.text.body};--sc-wa-font:${fonts.primary};">
        ...
        <form class="profile-whatsapp-form sc-wa-form">
          ...
          <section id="profileWhatsappDepartments" class="sc-wa-card sc-wa-departments"><div class="sc-wa-departments-head"><div><h4>Départements qui vous intéressent</h4><p>Choisissez les univers de produits dont vous souhaitez recevoir les nouveautés.</p></div><button type="button" class="sc-wa-select-all" id="profileWhatsappSelectAll">${allDepartmentsSelected ? 'Tout désélectionner' : 'Tout sélectionner'}</button></div><div class="sc-wa-department-grid">${departmentChoices}</div></section>
          ...
          <div class="sc-wa-actions"><button class="sc-wa-save" type="submit"><i class="fab fa-whatsapp"></i> Enregistrer mes préférences</button>${state.status === 'active' ? '<button class="sc-wa-confirm" type="button" data-whatsapp-confirm><i class="fab fa-whatsapp"></i> Recevoir une confirmation</button><button class="sc-wa-disable" type="button" data-whatsapp-unsubscribe>Tout désactiver</button>' : ''}</div>
        </form>
      </section>`;
  }

  async saveWhatsAppPreferences({ unsubscribe = false, requestConfirmation = false } = {}) {
    ...
    if (marketingOptIn && marketingDepartments.length === 0) {
      this.authManager.showToast('Sélectionnez au moins un département pour recevoir les nouveautés.', 'error');
      this.modal.querySelector('#profileWhatsappDepartments')?.scrollIntoView({ behavior: 'smooth', block: 'center' });
      return;
    }
    // Envoyer les deux noms de propriété pour rester compatible avec la
    // Cloud Function actuellement déployée (ancienne API: marketingCategories)
    // et la nouvelle API (marketingDepartments).
    const result = await this.callWhatsAppPreferences({ method: 'POST', body: {
      phone,
      serviceOptIn,
      marketingOptIn,
      requestConfirmation,
      marketingDepartments,
      marketingCategories: marketingDepartments
    } });
  ...
  attachEvents() {
    ...
    this.modal.querySelector('#profileWhatsappSelectAll')?.addEventListener('click', (event) => {
      const button = event.currentTarget;
      const checkboxes = Array.from(this.modal.querySelectorAll('[name="whatsapp-department"]'));
      const selectAll = checkboxes.some((checkbox) => !checkbox.checked);
      checkboxes.forEach((checkbox) => { checkbox.checked = selectAll; });
      button.textContent = selectAll ? 'Tout désélectionner' : 'Tout sélectionner';
    });
    ...
    this.modal.querySelector('[data-whatsapp-confirm]')?.addEventListener('click', async () => {
      try {
        await this.saveWhatsAppPreferences({ requestConfirmation: true });
      } catch (error) {
        this.authManager.showToast(error.message || 'Impossible d’envoyer la confirmation WhatsApp.', 'error');
      }
    });
