...
class FooterComponent {
  ...
  render() {
    ...
    const copyrightText = this.config?.copyright?.custom || 
      `© ${year} ${this.config?.companyName || 'Smart Cut Services'}. ${this.config?.copyright?.text || 'Tous droits réservés'}`;
  ...
  renderInfoLinks() {
    const staticLinks = [
      { title: 'Catalogue', link: './catalogue.html' },
      { title: 'Imprimerie en ligne', link: './printing-hub.html' },
      { title: 'Marketplace vendeurs', link: './vendor-marketplace.html' }
    ];

    const seen = new Set();
    const mergedLinks = [...this.infoLinks, ...staticLinks].filter((link) => {
      const key = `${link?.title || ''}::${link?.pageId || link?.link || ''}`;
