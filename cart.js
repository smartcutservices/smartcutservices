...
import { auth, authReadyPromise, db } from './firebase-init.js?v=20260908-12';
...
class CartManager {
  ...
  async openCheckout(cartData) {
    ...
    try {
      const module = await import('./checkout.js?v=20260908-12');
  ...
  addItem(item) {
    ...
    if (Number.isInteger(editCartIndex) && editCartIndex >= 0 && editCartIndex < this.cart.length) {
      const { editCartIndex: _ignored, ...replacement } = item;
      this.cart[editCartIndex] = { ...replacement, addedAt: this.cart[editCartIndex].addedAt || Date.now(), updatedAt: Date.now() };
      this.saveCart();
      this.showNotification('Création personnalisée mise à jour');
      return;
    }
  ...
  isTechnicalOptionValue(value) {
    ...
    return (
      ...
      || normalized.startsWith('personalization/')
  ...
  openLikedProduct(productId) {
    if (!productId) return;
    this.closeCartModal();
    import('./product-modal.js?v=20260901-1')
      .then((module) => {
  ...
  renderEmptyCart(colors, fonts) {
    return `
      ...
      ">
        ...
        <a class="continue-shopping-btn empty-cart-browse" href="./catalogue.html" style="
          background: ${colors.background.button};
          color: ${colors.text.button};
          border: 1px solid ${colors.background.button};
      ...
      return `
        ...
        ">
          ...
          <div style="flex: 1;">
            ...
            ${item.personalizationConfig ? `<button class="edit-personalization" type="button" data-index="${index}" style="border:1px solid rgba(15,17,17,.18);background:#fff;border-radius:.4rem;padding:.35rem .55rem;font:inherit;font-size:.72rem;font-weight:700;cursor:pointer;margin:.2rem 0 .35rem;"><i class="fas fa-pen" aria-hidden="true"></i> Modifier la création</button>` : ''}
  ...
  attachModalEvents() {
    ...
    this.modal.querySelectorAll('.edit-personalization').forEach((btn) => {
      btn.addEventListener('click', (event) => {
        event.stopPropagation();
        const index = Number(btn.dataset.index);
        const item = this.cart[index];
        if (!item?.personalizationConfig) return;
        sessionStorage.setItem('smartcut-personalization-edit', JSON.stringify({
          cartIndex: index,
          designId: item.designId,
          personalizationConfig: item.personalizationConfig
        }));
        window.location.href = `./personalization.html?design=${encodeURIComponent(item.designId || '')}`;
      });
    });
