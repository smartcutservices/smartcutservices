...
export default class HealthTeleconsultation {
  ...
  renderShell() {
    this.root.innerHTML = `<div class="health-shell telehealth-shell">
      ...
      </div><div class="health-hero-visual health-hero-visual--photo" role="img" aria-label="Illustration téléconsultation"><img src="./assets/health/heroes/doctor-hero-v1.png" alt="Médecin en téléconsultation" loading="eager"></div></div></section>
  ...
  async book(event) {
    ...
    try {
      ...
      const appointment = await callHealth('healthBookAppointment', { method:'POST', authRequired:true, body:{ slotId:form.get('slotId'), reason:form.get('reason'), patientName:form.get('patientName'), patientAge:Number(form.get('patientAge')), patientSex:form.get('patientSex'), specialtyCode:this.specialtyCode, planCode:this.planCode } });
