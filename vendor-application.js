...
const NEXT_DESTINATIONS = {
  'auto-parts': {
    href: './auto-parts-vendor.html',
    label: 'Continuer vers Auto & Parts',
    approvedMessage: 'Votre compte vendeur est actif. Continuez vers Auto & Parts pour demander votre spécialisation et publier vos premières pièces.'
  }
};

function getNextDestination() {
  const key = new URLSearchParams(window.location.search).get('next') || '';
  return NEXT_DESTINATIONS[key] || null;
}
...
class VendorApplicationPage {
  constructor(containerId = 'vendor-application-root') {
    ...
    this.selectedPlan = '';
    this.nextDestination = getNextDestination();
      ...
      <section style="max-width:980px;margin:0 auto;padding:1.2rem 1rem 0;">
        <div style="border-radius:2rem;border:1px solid rgba(31,30,28,0.08);background:linear-gradient(180deg,#fffdf9 0%,#f5eee2 100%);box-shadow:0 24px 54px rgba(31,30,28,0.08);overflow:hidden;">
          <div style="padding:clamp(1.5rem,4vw,2.6rem);border-bottom:1px solid rgba(31,30,28,0.08);background:radial-gradient(circle at top right, rgba(198,167,94,0.18), transparent 26%), linear-gradient(135deg, rgba(255,255,255,0.96), rgba(249,243,232,0.96));">
            ...
            <h1 style="margin:1rem 0 0;font-family:'Amazon Ember',Arial,sans-serif;font-size:clamp(2.3rem,5vw,4rem);line-height:.92;color:#0F1111;">${isApproved ? 'Candidature validée' : this.escape(this.formSettings.title || DEFAULT_FORM_SETTINGS.title)}</h1>
            ...
            ${this.application ? `
              <div style="margin-top:1rem;display:grid;grid-template-columns:repeat(auto-fit,minmax(170px,1fr));gap:.75rem;max-width:760px;">
                <div style="border-radius:1rem;border:1px solid rgba(31,30,28,0.08);background:rgba(255,255,255,0.72);padding:.85rem 1rem;">
                  <strong style="display:block;color:#0F1111;font-size:.85rem;">Soumise le</strong>
                  <span style="display:block;margin-top:.25rem;color:#6E6557;">${this.escape(submittedAt)}</span>
                </div>
                <div style="border-radius:1rem;border:1px solid rgba(31,30,28,0.08);background:rgba(255,255,255,0.72);padding:.85rem 1rem;">
                  <strong style="display:block;color:#0F1111;font-size:.85rem;">Derniere revue</strong>
                  <span style="display:block;margin-top:.25rem;color:#6E6557;">${this.escape(reviewedAt)}</span>
                </div>
                <div style="border-radius:1rem;border:1px solid rgba(31,30,28,0.08);background:rgba(255,255,255,0.72);padding:.85rem 1rem;">
                  <strong style="display:block;color:#0F1111;font-size:.85rem;">Actif comme vendeur depuis</strong>
          ...
          <div style="padding:clamp(1.2rem,4vw,2rem);">
            ${!this.user ? `
              <div style="border-radius:1.25rem;border:1px solid rgba(198,167,94,0.22);background:rgba(198,167,94,0.08);padding:1.15rem 1.2rem;">
                ...
                <button type="button" id="vendorSignInBtn" style="margin-top:1rem;border:none;border-radius:999px;background:#0F1111;color:#F8F5EF;padding:.9rem 1.15rem;font-weight:800;cursor:pointer;">Se connecter</button>
              </div>
            ` : this.application ? `
              <div style="display:grid;gap:1rem;">
                <div style="border-radius:1.25rem;border:1px solid rgba(31,30,28,0.08);background:#fff;padding:1.15rem 1.2rem;">
                  <strong style="display:block;color:#0F1111;font-size:1.02rem;">Statut actuel</strong>
                  <p style="margin:.55rem 0 0;color:#6E6557;line-height:1.8;">
                    ${isApproved
                      ? (this.nextDestination?.approvedMessage || 'Votre demande a ete validee. Votre dashboard vendeur est maintenant disponible pour ajouter vos produits.')
                      : status === 'rejected'
                ...
                ${this.application?.adminNote ? `
                  <div style="border-radius:1rem;border:1px solid rgba(31,30,28,0.08);background:#fff;padding:1rem 1.05rem;">
                    <strong style="display:block;color:#0F1111;margin-bottom:.35rem;">Note admin</strong>
                ...
                ${status === 'approved' ? `
                  <div style="display:grid;gap:.9rem;">
                    ...
                    <a href="${this.nextDestination?.href || VENDOR_DASHBOARD_URL}" id="vendorDashboardAccessBtn" style="display:inline-flex;align-items:center;gap:.65rem;text-decoration:none;border-radius:999px;background:#0F1111;color:#F8F5EF;padding:1rem 1.2rem;font-weight:800;">
                      <i class="fas fa-store"></i>
                      ${this.nextDestination?.label || 'Acceder a mon dashboard'}
                    </a>
                    </div>
                  </div>
                ` : ''}
            ...
            ` : shouldChoosePlan ? this.renderPlanSelection() : `
              <form id="vendorApplicationForm" style="display:grid;gap:1rem;">
                ...
                <div style="display:flex;align-items:center;gap:.8rem;flex-wrap:wrap;padding-top:.5rem;">
                  <button type="submit" style="border:none;border-radius:999px;background:#0F1111;color:#F8F5EF;padding:1rem 1.2rem;font-weight:800;cursor:pointer;">
  ...
  renderSelectedPlanNotice() {
    ...
    return `
      <div style="border-radius:1.1rem;border:1px solid rgba(198,167,94,0.24);background:rgba(198,167,94,0.1);padding:1rem;display:flex;justify-content:space-between;gap:1rem;align-items:center;flex-wrap:wrap;">
        <div>
          <strong style="display:block;color:#0F1111;">Plan selectionne: ${this.escape(plan.label)}</strong>
        ...
        <button type="button" id="changeVendorPlanBtn" style="border:1px solid rgba(31,30,28,0.14);border-radius:999px;background:#fff;color:#0F1111;padding:.75rem 1rem;font-weight:800;cursor:pointer;">Changer de plan</button>
      </div>
    `;
  }
    ...
    return `
      <section style="display:grid;gap:1.2rem;">
        <div style="border-radius:1.35rem;border:1px solid rgba(31,30,28,0.08);background:#fff;padding:1.15rem;">
          <small style="display:block;color:#FFA41C;text-transform:uppercase;letter-spacing:.14em;font-weight:800;margin-bottom:.5rem;">Choisissez votre plan</small>
          <h2 style="margin:0;font-family:'Amazon Ember',Arial,sans-serif;font-size:2.3rem;line-height:1;color:#0F1111;">Vendez avec plus de visibilite</h2>
  ...
  renderPlanCard(plan) {
    return `
      <article style="position:relative;border-radius:1.35rem;border:1px solid ${plan.highlight ? 'rgba(198,167,94,0.5)' : 'rgba(31,30,28,0.08)'};background:${plan.highlight ? 'linear-gradient(180deg,#0F1111,#3A3328)' : '#fff'};color:${plan.highlight ? '#F8F5EF' : '#0F1111'};padding:1.15rem;box-shadow:${plan.highlight ? '0 22px 44px rgba(31,30,28,0.22)' : '0 14px 34px rgba(31,30,28,0.08)'};display:grid;gap:1rem;">
        ${plan.highlight ? `<span style="position:absolute;top:1rem;right:1rem;border-radius:999px;background:#FFA41C;color:#0F1111;padding:.35rem .65rem;font-size:.72rem;font-weight:900;">Recommande</span>` : ''}
        <div>
        ...
        <ul style="margin:0;padding:0;list-style:none;display:grid;gap:.65rem;">
          ${plan.features.map((feature) => `
            <li style="display:flex;gap:.55rem;align-items:flex-start;line-height:1.55;">
              <i class="fas fa-check" style="margin-top:.25rem;color:${plan.highlight ? '#FFA41C' : '#14532D'};"></i>
              <span>${this.escape(feature)}</span>
            </li>
          `).join('')}
        </ul>
        <button type="button" data-select-vendor-plan="${this.escape(plan.id)}" style="border:none;border-radius:999px;background:${plan.highlight ? '#FFA41C' : '#0F1111'};color:${plan.highlight ? '#0F1111' : '#F8F5EF'};padding:1rem 1.1rem;font-weight:900;cursor:pointer;">
  ...
  renderStatusSummary() {
    ...
    return configuredFields.map((field) => {
      ...
      return `
        <div style="border-radius:1rem;border:1px solid rgba(31,30,28,0.08);background:#fff;padding:1rem 1.05rem;">
          <strong style="display:block;color:#0F1111;margin-bottom:.3rem;">${this.escape(field.label || field.id)}</strong>
          ...
          <div class="${this.uniqueId}-options">
            ${options.map((option, index) => `
              <label class="${this.uniqueId}-option">
                ...
                <span style="color:#0F1111;">${this.escape(option)}</span>
              </label>
            `).join('')}
          </div>
        </div>
    ...
    if (field.type === 'checkbox') {
      return `
        <label class="${this.uniqueId}-option" style="margin-top:.2rem;">
          ...
          <span style="color:#0F1111;line-height:1.7;">${this.escape(field.label || field.id)}</span>
        </label>
      `;
    }
  ...
  showSuccessModal() {
    ...
    overlay.innerHTML = `
      ...
      ">
        ...
        <h3 style="margin:1rem 0 0;font-family:'Amazon Ember',Arial,sans-serif;font-size:2rem;">Demande envoyee</h3>
        ...
        <div style="display:flex;justify-content:flex-end;margin-top:1.2rem;">
          <button type="button" id="vendorApplicationSuccessClose" style="border:none;border-radius:999px;background:#0F1111;color:#F8F5EF;padding:.9rem 1.15rem;font-weight:800;cursor:pointer;">
            Compris
          </button>
        </div>
      </div>
