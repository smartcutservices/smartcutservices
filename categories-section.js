...
class CategoriesSection {
    ...
    isProductVisible(product) {
        return isPublicProductVisible(product);
    }
    
    getProductImages(productData) {
        return getResolvedProductImages(productData, this.options.imageBasePath);
    }
    ...
    loadProducts() {
        ...
        try {
            const handleProducts = async (docs) => {
                if (!Array.isArray(docs) || docs.length === 0) {
                    this.state.allProducts = [];
                } else {
                    const rawProducts = docs.map((data) => {
            ...
            if (this.options.collectionName === 'products') {
                this.unsubscribeProducts = subscribePublicProducts(handleProducts, {
                    maxPerCollection: this.options.maxRealtimeProducts
                });
            } else {
                ...
                this.unsubscribeProducts = onSnapshot(q, async (snapshot) => {
                    await handleProducts(snapshot.docs.map(doc => ({
                        id: doc.id,
                        ...doc.data()
                    })));
                }, (error) => {
        ...
        card.innerHTML = `
            <div class="product-image-container-${this.uniqueId}">
                ${mainImage ? `
                    ...
                    >
                ` : `
                    <div style="width:100%;height:100%;display:flex;align-items:center;justify-content:center;color:var(--text-subtitle, #7A746B);">
